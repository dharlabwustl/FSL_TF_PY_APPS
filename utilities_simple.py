...
# #        print(imagefile_levelset)
#     #    npad = ((0, 0), (difference_size//2, difference_size//2), (0, 0))
#         size_diff=np.abs(image_nib_nii_file_data.shape[1]-512)
...
#         else :
#             size_diff=int(size_diff/2)
#             image_nib_nii_file_data=image_nib_nii_file_data[0:image_nib_nii_file_data.shape[0], size_diff:(image_nib_nii_file_data.shape[1]-size_diff-1), 0:image_nib_nii_file_data.shape[2]] 
...
#             size_diff=int(size_diff/2)
#             npad = ((0, 0), (size_diff-1, size_diff+1), (0, 0)) 
# #             image_nib_nii_file_data=np.pad(image_nib_nii_file_data, pad_width=npad, mode='constant', constant_values=np.min(image_nib_nii_file_data)) #image_nib_nii_file_data[0:image_nib_nii_file_data.shape[0],0+(difference_size//2):image_nib_nii_file_data.shape[1]-(difference_size//2),0:image_nib_nii_file_data.shape[2]]
#             print("I am EVEN")
#         else :
#             size_diff=int(size_diff/2)
#             npad = ((0, 0), (size_diff, size_diff+1), (0, 0)) 
#         image_nib_nii_file_data=np.pad(image_nib_nii_file_data, pad_width=npad, mode='constant', constant_values=np.min(image_nib_nii_file_data)) 
#             #image_nib_nii_file_data[0:image_nib_nii_file_data.shape[0],0+(difference_size//2):image_nib_nii_file_data.shape[1]-(difference_size//2),0:image_nib_nii_file_data.shape[2]]
#     return image_nib_nii_file_data
#########################################
# def resizeinto_512by512xxx(image_nib_nii_file_data):
# #     image_nib_nii_file=nib.load(original_file)  #,header=False)
# #     image_nib_nii_file_data=image_nib_nii_file.get_fdata() #
# #     image_levelset_data=nib.load(levelset_file).dataobj.get_unscaled()
# #     array_mask1=nib.load(levelset_file)
...
def latex_start(filename):
    ...
    file1.writelines("\\usepackage{geometry}\n")
    file1.writelines("\\geometry{a4paper,total={200mm,300mm},left=1mm,right=21mm,top=1mm,bottom=0.001mm}\n")
...
def contrast_stretch_np(img,threshold_id):
    ...
    return ct_image
def contrast_stretch_np_range(img,in_range1):
    ct_image=exposure.rescale_intensity(img , in_range=(in_range1[0], in_range1[1]))
...
def saveslicesofnumpy3D_non_zero(img_gray_data,savefilename="",savetodir=""):
    unique_vals=np.unique(img_gray_data.flatten())
    if len(unique_vals) > 3:
        if np.min(img_gray_data)>=0:
            img_gray_data=contrast_stretch_np(img_gray_data,2) *255 #(img_gray_data)
        if np.min(img_gray_data)<0:
            img_gray_data=contrast_stretch_np(img_gray_data,1) *255 #(img_gray_data)           
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x] )
    else:
        ...
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x]*255 )

def saveslicesofnumpy3D_non_zero_image(img_gray_data,savefilename="",savetodir=""):
    unique_vals=np.unique(img_gray_data.flatten())
    if len(unique_vals) > 3:
        if np.min(img_gray_data)>=0:
            img_gray_data=contrast_stretch_np(img_gray_data,2) *255 #(img_gray_data)
        ...
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,"image_"+os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x] )
    else:
        ...
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,"image_"+os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x]*255 )
                
def saveslicesofnumpy3D_non_zero_mask(img_gray_data,savefilename="",savetodir=""):
    unique_vals=np.unique(img_gray_data.flatten())
    if len(unique_vals) > 3:
        if np.min(img_gray_data)>=0:
            img_gray_data=contrast_stretch_np(img_gray_data,2) *255 #(img_gray_data)
        ...
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,"mask_"+ os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x] )
    else:
        ...
        for x in range(img_gray_data.shape[2]):
            if np.sum(img_gray_data[:,:,x]>0):
                slice_num="{0:0=3d}".format(x)
                cv2.imwrite(os.path.join(savetodir,"mask_"+os.path.basename(savefilename).split(".nii")[0].replace(".","_")+str(slice_num)+".png" ),img_gray_data[:,:,x]*255 )                
