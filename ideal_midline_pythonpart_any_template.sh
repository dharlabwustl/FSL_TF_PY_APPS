...
GRAYSCALENIFTI_FILE=${1} ##${input_filename} #$input_for_BET/
output_directory=$(dirname ${GRAYSCALENIFTI_FILE})
OUTPUT_DIRECTORY=${output_directory}
#TRANSFORMED_MASK_DIRECTORY=${2} #${output_directory}
#basename_grayfilenifti=$(basename -- $GRAYSCALENIFTI_FILE)
transformed_output_file=${2} #$TRANSFORMED_MASKFILE_PREFIX${basename_grayfilenifti%.nii*}$MASKFILE_EXTENSION
...
run_fit_line_to_midlinepixels_ORF_sh "$GRAYSCALENIFTI_FILE" $transformed_output_file  "$OUTPUT_DIRECTORY"  "${method_type}" "${method_type_name}"
