...
def resizeinto_512by512(image_data):
    """
    Resize a 3D image (X, Y, Z) to shape (512, 512, Z)
    by symmetric padding or cropping in X and Y dimensions.
    """
    print('I am in utilities_simple_trimmed.py')

    x, y, z = image_data.shape
    temp_array = np.copy(image_data)

    # ---- Padding if smaller than 512 ----
    pad_x = max(0, 512 - x)
    pad_y = max(0, 512 - y)
    ...
    if pad_x > 0 or pad_y > 0:
        ...
        temp_array = np.pad(temp_array, pad_width=npad, mode='constant', constant_values=np.min(image_data))

    # ---- Cropping if larger than 512 ----
    new_x, new_y = temp_array.shape[:2]

    crop_x = max(0, new_x - 512)
    crop_y = max(0, new_y - 512)

    if crop_x > 0:
        start_x = math.floor(crop_x / 2)
        temp_array = temp_array[start_x:start_x + 512, :, :]

    if crop_y > 0:
        start_y = math.floor(crop_y / 2)
        temp_array = temp_array[:, start_y:start_y + 512, :]
...
def copy_nifti_parameters_sh():
    ...
    return "x"
def copy_nifti_parameters_py(file,file1,output_directoryname):
    # file = sys.argv[1]
    # file1=sys.argv[3]
    # output_directoryname=  sys.argv[2]
    #    files=glob.glob(directoryname+"/*_levelset.nii.gz")
    ...
    #        new_header['pixdim']= target_nii.header['pixdim']
    array_img = nib.Nifti1Image(target_nii.get_fdata(),affine=affine_source_nii.affine, header=new_header)
    nib.save(array_img, os.path.join(output_directoryname,'with_copied_params_'+target_save))
    return "x"
...
def latex_additionalPackages(filename,packagename_list):
    file1 = open(filename,"a")
    file1.writelines("\n")
    for package_name in packagename_list:
        package_name_str="\\usepackage{{"+package_name+"}}\n"
        file1.writelines(package_name_str ) ##f"\\usepackage{{package_name}}\n")
    return file1
...
# def df_to_latex_1(df):
#     """
#     Converts a Pandas DataFrame to a LaTeX longtable format for multi-page tables.
#
#     Parameters:
#         df (pd.DataFrame): The DataFrame to convert.
#
#     Returns:
#         str: The LaTeX longtable representation of the DataFrame.
#     """
...
#         return text
#
#     headers = [escape_latex(str(col)) for col in df.columns]  # Convert headers to string
#     rows = [[escape_latex(str(cell)) for cell in row] for row in df.values]  # Convert each cell
#
#     # Begin LaTeX longtable
#     latex = "\\begin{longtable}{" + " | ".join(["l"] * len(headers)) + "}\n\\hline\n"
#
#     # First page header
#     latex += " & ".join(headers) + " \\\\\n\\hline\n"
#     latex += "\\endfirsthead\n\n"
...
def df_to_latex_2(df,colmn_width,thiscaption='table'):
    ...
    for each_header in headers:
        new_header.append("\\rotatebox{90}{"+each_header+"}")
    headers=new_header
    rows = [[escape_latex(str(cell)) for cell in row] for row in df.values]
